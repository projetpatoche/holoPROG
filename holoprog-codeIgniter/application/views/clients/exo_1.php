    ...
    <div class="col-md-3">
     ...
     <div class="col-md-6">
        <form method="post" action="<?php echo base_url();?>index.php/client_c/correctionExo/<?php echo $exercice->id_exercice?> " >
            <div class="">
			<?php for($i=0;$i<$exercice->taille_exo;$i++){//liste pour le melange
				$liste[]=$i;
			}
			if($exercice->difficulte==2){
				shuffle($liste);?>
			<ul class="sortable">
			<?php } ?>
				<?php for($i=0;$i<$exercice->taille_exo;$i++){?>
					<div class="barriere_bloc_1">
						<SELECT class="select_barriere_bloc_1" name="<?php echo $liste[$i] ?>">
							<OPTION value="1"><?php echo $select[''.$liste[$i].''][0]?></OPTION>
							<OPTION value="2"><?php echo $select[''.$liste[$i].''][1]?></OPTION>
							<OPTION value="3"><?php echo $select[''.$liste[$i].''][2]?></OPTION>
						</SELECT>
